...
export const getExampleCashOutTrxData: () => CashOutTrxData = () => {
    const generateDate = new Date().toISOString();
    const dateNow = generateDate.split("T")[0];
    const timeNow = generateDate.split("T")[1].split(".")[0];

    const randomData =
        randomExampleDataArray[
            Math.floor(Math.random() * randomExampleDataArray.length)
        ];
    const newData: CashOutTrxData = {
...
const randomExampleDataArray: CashOutTrxData[] = [
    {
        transactionStartDateTime: "04.04.2023 12:15:13",
        deviceName: "RNET3232",
